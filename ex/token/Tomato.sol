...
contract Tomato is ERC20, Ownable {
    ...
    uint256 public buyFeesRate;
    uint256 public sellFeesRate;
    ...
    constructor(
        ...
        uint256[6] memory feeSettings // rewards, liquidity, burn, buyFeesRate, sellFeesRate, baseFees
    ) ERC20(name_, symbol_) {
        wrap = new Wrap();
        ...
        buyFeesRate = feeSettings[3];
        sellFeesRate = feeSettings[4];
    ...
    function setBuyFeeRate(uint256 value) external onlyOwner {
        require(value <= 100, "max fee rate is 100");
        buyFeesRate = value;
    }

    function setSellFeeRate(uint256 value) external onlyOwner {
        require(value <= 100, "max fee rate is 100");
        sellFeesRate = value;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            uint256 fees = amount.mul(baseFees).div(100);
            if (automatedMarketMakerPairs[to]) {
                fees = fees.mul(sellFeesRate).div(100);
            }else if (automatedMarketMakerPairs[from]) {
                fees = fees.mul(buyFeesRate).div(100);
            }
