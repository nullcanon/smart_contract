...
contract StakeReward is Ownable, ReentrancyGuard {
     ...
     */
    constructor() {
        STACKE_REWARD_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == STACKE_REWARD_FACTORY, "Not factory");
...
// File: contracts/StakeRewardFactory.sol

contract StakeRewardFactory is Ownable {
    event NewStakeRewardContract(address);
     ...
     * @param _numberBlocksForUserLimit: block numbers available for user limit (after start block)
     * @param _admin: admin address with ownership
     * @return address of new smart stacke reward contract
     */
    function deployPool(
        IERC20Metadata _stakedToken,
        IERC20Metadata _rewardToken,
        ...
        bytes memory bytecode = type(StakeReward).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address stakeReward;

        assembly {
            stakeReward := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        StakeReward(stakeReward).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewStakeRewardContract(stakeReward);
    }
}