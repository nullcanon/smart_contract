...
contract PayChannel is AdminableUpdateable{
    ...
    event RegisterGamed(uint256 gameId, uint256 startTime, uint256 endTime, uint256 index, string flag, uint256[] minLimit, uint256[] maxLimit);
    event ModifyGamed(uint256 gameId, uint256 startTime, uint256 endTime, uint256 index, string flag, uint256[] minLimit, uint256[] maxLimit);
    ...
    event BankChange(address indexed oldBank, address indexed newBank);

    function setBank(address _bank) public onlyOwner {
        emit BankChange(bankAddress, _bank);
        bankAddress = _bank;
    }

    function _isPayETH(address money) pure returns bool {
    ...
    function playGame(address _payMoney, uint256 _gameId, uint256 _resultId, uint256 _amount) public payable {
        ...
        GameInfo _info = gameList[_gameId];
    ...
    function registerGame(uint256 _gameId, uint256 _startTime, uint256 _endTime, string memory _flag, 
        ...
        emit RegisterGamed(gameId, startTime, endTime, flag, minLimit, maxLimit);
    }
    ...
    function modifyGame(uint256 _gameId, uint256 _startTime, uint256 _endTime, string memory _flag, 
        ...
        _info.maxLimit = _maxLimit;
        gameList[_gameId] = _info;
        emit ModifyGamed(gameId, startTime, endTime, flag, minLimit, maxLimit);
    }
   ...
   function getMessageHash(
    ...
    function claim(        
        ...
        require(verify(_to, _amount, _nonce, _signature), "signature verify faild");
        ...
        if( _isPayETH(_payMoney) ) {
            ...
            IERC20(_payMoney).transfer(msg.sender, _destory);
        }
