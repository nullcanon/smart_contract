...
library LibArrayForUint256Utils {
    ...
    function removeByIndexFast(uint256[] storage array, uint index) internal{
    	require(index < array.length, "ArrayForUint256: index out of bounds");
        array[index] = array[array.length - 1];
        array.pop();
    }

    function removeByValueFast(uint256[] storage array, uint256 value) internal{
        uint index;
        bool isIn;
        (isIn, index) = firstIndexOf(array, value);
        if(isIn){
          removeByIndexFast(array, index);
        }
    }
...
contract marketPlace is ReentrancyGuard , ERC1155Holder, Ownable{
    ...
    function cancelMarketItemErc1155(uint256 itemId) public nonReentrant {
        ...
        LibArrayForUint256Utils.removeByValueFast(orderMarketItemIds, itemId);
    ...
    function createMarketSaleErc1155(
        ...
        uint256 itemId
            ...
            uint tokenId = idToMarketItem[itemId].tokenId;
            Status sold = idToMarketItem[itemId].sold;
            require(msg.value >= priceMarket, "This Sale has alredy finnished");
            emit MarketItemSold(
                itemId,
                nftContract,
                tokenId,
            ...
            uint256 fee = priceMarket.mul(feeNumerator).div(feeDenominator);


            idToMarketItem[itemId].seller.transfer(priceMarket.sub(fee));
